				...
				<div class="col-sm-12">
					...
					<div class="ibox float-e-margins">
						<div class="ibox-title">
							<h5>编辑商品<small class="m-l-sm"></small></h5>
						</div>
									...
									<div class="tab-content" style="margin-top: 20px">
										<div id="tab-4" class="tab-pane active">
											...
											<div class="form-group">
												<label class="col-sm-2 control-label"><i class="icon iconfont icon-need">&#xe657;</i>类型：</label>
												<div class="col-sm-8">
													<input value="<c:if test="${product.type==0}">普通商品</c:if><c:if test="${product.type==1}">企业专区</c:if>" class="form-control" type="text" disabled>
												</div>
											</div>
											...
											<div class="form-group">
												...
												<div class="col-sm-8">
													<input id="sellDate" name="sellDate" class="form-control" type="text" value="<fmt:formatDate value="${product.sellDate}" pattern="yyyy-MM-dd HH:mm:ss"/>" >
												</div>
											</div>
											...
											<div class="form-group">
												...
												<div class="col-sm-9">
													<input id="subName" name="subName" value="${product.subname }" class="form-control" type="text">
												</div>
											</div>
											<div class="form-group">
												<label class="col-sm-2 control-label"><i class="icon iconfont icon-need">&#xe657;</i>商品分类: </label>
												<div class="col-sm-8">
													<select class="form-control" name="productCategoryId" size="1">
														<c:forEach items="${productCategoryList}" var="productCategory">
															<option <c:if test="${product.productCategoryId==productCategory.id}">selected="selected"</c:if> value="${productCategory.id}">${productCategory.name}</option>
														</c:forEach>
													</select>
												</div>
											</div>
											...
											<div class="form-group">
												...
												<div class="col-sm-8">
													<input class="form-control" type="text" name="showUrl" value="${product.showUrl }" id="showUrl" readonly="readonly" style="display: inline-block;width: 200px;line-height: 34px;height: 34px;float: left;">
													<label for="file">
											...
											<div class="form-group">
												...
												<div class="col-sm-8">
													<input class="form-control" type="text" name="longShowUrl" value="${product.longShowUrl }" id="longShowUrl" readonly="readonly" style="display: inline-block;width: 200px;line-height: 34px;height: 34px;float: left;">
													<label for="file1">
														<a class="btn btn-primary upload-btn"><i class="icon iconfont">&#xe692;</i> 选择图片</a>
													</label>
											...
											<div class="form-group">
												...
												<div class="col-sm-8">
													<input id="introduce" name="introduce" value="${product.introduce }" class="form-control" type="text">
												</div>
											</div>
											<div class="form-group">
												<label class="col-sm-2 control-label">排序：</label>
												<div class="col-sm-8">
													<input id="orders" name="orders" value="${product.orders }" class="form-control" type="text">
												</div>
											</div>
											<div class="form-group">
												<label class="col-sm-2 control-label">备注：</label>
												<div class="col-sm-8">
													<input id="memo" name="memo" value="${product.memo }" class="form-control" type="text">
												</div>
											</div>

											<div class="form-group">
												<label class="col-sm-2 control-label"><i class="icon iconfont icon-need">&#xe657;</i>标签：</label>
												<div class="col-sm-8">
													<div class="checkbox i-checks">
														<c:forEach items="${tagList}" var="tag">
															<label><input type="checkbox" <c:forEach items="${productTagList }" var="productTag"><c:if test="${productTag.tagId==tag.id }">checked="checked"</c:if></c:forEach> value="${tag.id}" name="tagIds">&nbsp;${tag.name}</label>
														</c:forEach>
													</div>
												</div>
											</div>
												...
												<div class="col-sm-8">
													<div class="checkbox i-checks">
														<c:forEach items="${componentList}" var="component">
															<label><input type="checkbox" <c:forEach items="${productComponentList }" var="productComponent"><c:if test="${productComponent.componentId==component.id }">checked="checked"</c:if></c:forEach> value="${component.id}" name="componentIds">&nbsp;${component.name}</label>
														</c:forEach>
													</div>
												</div>
											</div>
												...
												<div class="col-sm-8">
													<div class="checkbox i-checks">
														<label>
															...
															<input id="isMarketableCheck" <c:if test="${product.isMarketable==1 }">checked="checked"</c:if> value="1" type="checkbox"> <i></i> 上架</label>
														<label>
															<input type="hidden" id="isTop" name="isTop" value="0">
															<input id="isTopCheck" <c:if test="${product.isTop==1 }">checked="checked"</c:if> value="1" type="checkbox"> <i></i> 置顶</label>
													</div>
												</div>
											</div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-4">
                                                    <select id="sweetness" class="form-control m-b" name="sweetness">
                                                        <option value="0">请选择甜度</option>
                                                        <option <c:if test="${product.sweetness==1}"> selected </c:if> value="1">1</option>
                                                        <option <c:if test="${product.sweetness==2}"> selected </c:if> value="2">2</option>
                                                        <option <c:if test="${product.sweetness==3}"> selected </c:if> value="3">3</option>
                                                        <option <c:if test="${product.sweetness==4}"> selected </c:if> value="4">4</option>
                                                        <option <c:if test="${product.sweetness==5}"> selected </c:if> value="5">5</option>
                                                    </select>
                                                </div>
                                            </div>
                                            ...
                                            <div class="form-group">
												...
												<div class="col-sm-8">
													<div class="checkbox i-checks">
														<label>
															<input id="isBirthdayCard1" <c:if test="${product.isBirthdayCard==1}"> checked </c:if> value="1" name="isBirthdayCard" type="radio"> <i></i> 是
														</label>
														<label>
															<input id="isBirthdayCard0" <c:if test="${product.isBirthdayCard==0}"> checked </c:if> value="0" name="isBirthdayCard" type="radio"> <i></i> 否
														</label>
													</div>
												</div>
											</div>
											...
											<div class="form-group">
												...
												<div class="col-sm-9">
													<div class="checkbox i-checks" id="guigeBox">
														<label id="sin">
															<input type="radio" value="1" name="optionsRadios" <c:if test="${product.spec == 0}">checked="checked"</c:if> onchange="testRadios()">
															<i></i> 统一规格
														</label>
														<label id="mul">
															<input type="radio" value="2" name="optionsRadios" <c:if test="${product.spec == 1}">checked="checked"</c:if> onchange="testRadios()">
															<i></i> 多规格
														</label>
													</div>
																					...
																					<div style="padding: 5px 8px;" class="div_content">
																						...
																						<div class="div_contentlist">
																							<c:forEach var="specName" items="${specNameList}" varStatus="status">
																								<ul specNameId="${specName.id}" class="Father_Title js_show_div">
																								...
																								<ul class="Father_Item${status.index}">
																									<c:forEach items="${specValueList}" var="specValue">
																										<c:if test="${specValue.specNameId == specName.id}">
																											<li class="li_width">
																												<label>
																													<input id="${specValue.id}" type="checkbox" class="chcBox_Width" value="${specValue.value}" />${specValue.value}<span class="li_empty"></span></label>
																											</li>
																										</c:if>
																									</c:forEach>
																									<div class="clear">
											...
											<c:if test="${product.spec == 1}">
												<div class="form-group" id="box-sep-2" style="display: block;">
													<label class="col-sm-2 control-label">价格&库存：</label>
													<div class="col-sm-9">
														<div style="font-size:14px;line-height:30px;font-weight: bold;">批量填充：</div>
														<div>
											...
											<c:if test="${product.spec == 0}">
												<div class="form-group single">
													...
													<div class="col-sm-8">
														<input id="price" name="price" value="${productStock.price }" class="form-control" type="text">
													</div>
												</div>
												<div class="form-group single">
													<label class="col-sm-2 control-label"><i class="icon iconfont icon-need">&#xe657;</i>库存：</label>
													<div class="col-sm-8">
														<input id="stock" name="stock" value="${productStock.stock }" class="form-control" type="text">
													</div>
												</div>
												<div class="form-group single">
													<label class="col-sm-2 control-label"><i class="icon iconfont icon-need">&#xe657;</i>最大抵扣使用积分：</label>
													<div class="col-sm-8">
														<input id="maxOffsetPoint" name="maxOffsetPoint" value="${productStock.maxOffsetPoint }" class="form-control" type="text">
													</div>
												</div>
											</c:if>
													...
													<tr>                                                                                                      									
														<td>																																						
															<div class='row'>																													
																...
																<div class='col-sm-6'>																										
																	<label class='btn btn-primary' for='inp-file'><i class='icon iconfont'>&#xe692;</i> 选择文件</label>			
																	<input name='inp-file' type='file' id='inp-file' onchange='upload();' style='display: none;'>					
																</div>																																													
															</div>																															
														</td>																																
														<td>																																
															<input id='imgOrders' name='imgOrders' class='form-control' type='text' value='${data.orders}'>													
														</td>																																
														<td>																																
		...
		<script>
            ...
            function save() {
               ...
               /* var $tagIds = $('input[name=tagIds]:checked');
                ...
                /*var sweetness = $("#sweetness option:selected").val();
                ...
                $.each($tr, function (i, n) {
                    ...
                    $.each(products, function (i, j) {
                        if (j.specName === spec_values) {
                            ...
                            var isDisabled = j.isDisabled;
                            var str = "";
                            if (isDisabled == 0) {
                                str = "<input name='isDisabled11' type='hidden' value='0' /> <i onclick='batchSet(this,\""+spec_values+"\")' data-toggle='modal' data-target='#batchSet' title='批量设置' style='cursor: pointer' class='icon iconfont'>&#xe64b;</i> <i onclick='setDisabled(this)' title='禁用' style='cursor: pointer' class='icon iconfont'>&#xe693;</i>";
							} else if (isDisabled == 1) {
                                str = "<input name='isDisabled11' type='hidden' value='1' /> <i onclick='batchSet(this,\""+spec_values+"\")' data-toggle='modal' data-target='#batchSet' title='批量设置' style='cursor: pointer' class='icon iconfont'>&#xe64b;</i> <i onclick='setDisabled(this)' title='启用' style='cursor: pointer' class='icon iconfont'>&#xe694;</i>";
                                $(n).children('td[name=specImg]').children('input').attr('disabled', 'disabled');
                                $(n).children('td[name=specImg]').children('label').attr('disabled', 'disabled');
                                $(n).children('td[name=price]').children('input').attr('disabled', 'disabled');
                                $(n).children('td[name=stock]').children('input').attr('disabled', 'disabled');
                                $(n).children('td[name=people]').children('input').attr('disabled', 'disabled');
                                $(n).children('td[name=tableware]').children('input').attr('disabled', 'disabled');
                                $(n).children('td[name=size]').children('input').attr('disabled', 'disabled');
                                $(n).children('td[name=maxPoint]').children('input').attr('disabled', 'disabled');
							}
                            $(n).children('td[name=isDisabled]').html(str);
                        }
