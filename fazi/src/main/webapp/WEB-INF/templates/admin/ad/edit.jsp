		...
		<form id="dataForm" class="form-horizontal" style="padding-top: 30px;">
			<div class="form-group">
				...
				<div class="col-sm-8">
					...
					<input id="id" name="id" value="${ad.id}" class="form-control" type="hidden">
				</div>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label"><i class="icon iconfont icon-need">&#xe657;</i>广告图片：</label>
				<div class="col-sm-8">
					<input name="imgUrl" class="form-control" id="imgUrl" value="${ad.imgUrl}" readonly="readonly" style="display: inline-block;width:70%;line-height: 34px;height: 34px;float: left;">
			...
			<div id="imgDiv" class="form-group">
				...
				<div class="col-sm-8">
					<img src="${ad.imgUrl}" width="100px" />
				</div>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label">链接地址：</label>
				<div class="col-sm-8">
					<input id="link" name="link" value="${ad.link}" class="form-control">
			...
			<div class="form-group">
				...
				<div class="col-sm-8">
					<div class="radio i-checks">
						<label>
							<input type="radio" <c:if test="${ad.isShow==1}">checked="checked"</c:if> value="1" name="isShow"> <i></i> 是
						</label>
						<label>
							<input type="radio" <c:if test="${ad.isShow==0}">checked="checked"</c:if> value="0" name="isShow"> <i></i> 否
						</label>
					</div>
				</div>
			</div>
				...
				<div class="col-sm-8">
					<select class="form-control" name="adlocationId" size="1">
					   <c:forEach items="${adlocationList}" var="adlocation">
						  <option <c:if test="${ad.adLocationId==adlocation.id}">selected="selected"</c:if> value="${adlocation.id}">${adlocation.name}</option>
					   </c:forEach>
					</select>
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-8">
					<input id="orders" name="orders" value="${ad.orders}" class="form-control" type="text">
				</div>
			</div>
			<div class="form-group">
